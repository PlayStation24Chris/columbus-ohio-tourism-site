...
for (let j=0; j<locValues.length; ++j)
{
    L.marker([locValues[j].Lat, locValues[j].Long], {icon: testIcon}).addTo(map).on('click', function(){
        L.popup().setLatLng(
            [locValues[j].Lat,locValues[j].Long])
            .setContent("<div class=\"Pop-up \"><h1>"+locValues[j].Name+"</h1>" +"<h2>"+ locValues[j].FlavorText+"</h2>"+ "<p>Address: " + locValues[j].Address + "</p>Learn More!: <a  href=\"" + locValues[j].href + "\">Visit Here</a></div>").openOn(map);
    });
}
